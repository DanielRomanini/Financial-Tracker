...
@app.route("/transactions") #GETS A LIST OF ALL TRANSACTIONS
def transactions():
...
@app.route("/transactions/<type>") #GETS A LIST OF TRANSACTIONS ACCORDING TO SPECIFIC TYPE OF TRANSACTION
...
@app.route("/transactions/<type>/total") #GETS TOTAL OF TRANSACTIONS FOR WHICHEVER TYPE YOU WANT
...
@app.route("/transactions/date/<date>") #RETURNS TRANSACTIONS ON A SPECIFIC DATE
...
@app.route("/totals") #RETURNS TOTALS OF ALL TRANSACTIONS GROUPED BY TYPE
def totals():
    connection = sqlite3.connect("Transactions.db")
    cursor = connection.cursor()
...
@app.route("/summary") #Shows how much you've spent, how much you've gained, and the difference between those
def summary():
    connection = sqlite3.connect("Transactions.db")
    cursor = connection.cursor()
